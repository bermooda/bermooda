...
**Themes are not bundled in the repo.** `npm run setup` installs the default theme (`@bermooda/theme-default`) from a sibling checkout (`../theme-default`) or npm pack into `app/themes/default/`, installs that theme's npm dependencies, and sets `activeTheme` when `DATABASE_URL` is available. Optional plugin siblings under `../plugin-*` are copied when present but not enabled. Re-run `npm run extensions:install` after pulling new theme code. To only (re)install nested deps for extensions already on disk: `npm run extensions:install-deps`.
...
- **Extensions:** `app/themes/` and `app/plugins/` are empty in the repo and gitignored. `npm run setup` (or `npm run extensions:install`) installs the default theme from a sibling checkout or npm. Optional plugin siblings are copied when present but not enabled. `npm run build` runs `prebuild` → `extensions:install-deps` so nested deps exist for Vite; extension runtime deps are listed in `ssr.noExternal` and bundled into the server build.
