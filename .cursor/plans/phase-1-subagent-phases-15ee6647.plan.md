...
- **P1-C. Storage client API.** Create [app/core/storage/client/index.server.js](app/core/storage/client/index.server.js) with an S3-compatible wrapper reading `STORAGE_ENDPOINT`, `STORAGE_REGION`, `STORAGE_BUCKET`, `STORAGE_ACCESS_KEY`, `STORAGE_SECRET_KEY`, `STORAGE_PUBLIC_URL`. Expose `putObject`, `getObjectUrl`, `deleteObject`. Add vars to [.env.example](.env.example). Write [docs/storage.md](docs/storage.md) with setup steps, local-dev behavior, failure modes.
...
- **P9-5. Finalize [docs/storage.md](docs/storage.md)** — S3-compatible provisioning, env vars, local fallback, failure modes.
