...
- **P7-1. sample-analytics plugin** — [app/plugins/sample-analytics/manifest.js](../app/plugins/sample-analytics/manifest.js), `index.server.js` (hooks `order.created` → append to `PluginData`), `admin/routes.js` (recent-events page), `blocks/product.afterDescription.jsx`, `i18n/en.json`.
...
- **P9-6. E2E smoke run.** Execute on a freshly seeded DB (`npm run setup && npm run seed`):
  1. Boot dev server; confirm 1 admin, default theme active, sample plugin enabled.
