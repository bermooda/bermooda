...
- **Tools** grouped by intent, not 1:1 every REST route — e.g. `list_products`, `upsert_product`, `get_settings`, `update_settings`, `list_orders`, `update_order_status`, `import_products_csv`, `setup_shop` (orchestrates settings + seed catalog). Reporting tools map to `/api/admin/v1/reports/*` (overview, sales slices, ops, customers, inventory, exports, dashboard).
